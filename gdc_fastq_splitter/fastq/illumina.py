...
class IlluminaSequenceIdentifier(base.SequenceIdentifier):
    ...
    def from_string(cls, seqid):
        _keys = ['instrument_name', 'run', 'flowcell', 'lane',
            'tile', 'x_coord', 'y_coord', 'read_pair', 'pf',
            'bits', 'index']
    ...
    @staticmethod
    def is_valid(seqid):
        if seqid.count(':') != 9: return False
        elif seqid.count(' ') != 1: return False 
        return True
...
class IlluminaSequenceIdentifierNoBarcode(base.SequenceIdentifier):
    ...
    def from_string(cls, seqid):
        _keys = ['instrument_name', 'run', 'flowcell', 'lane',
            'tile', 'x_coord', 'y_coord', 'read_pair']
    ...
    @staticmethod
    def is_valid(seqid):
        if seqid.count(':') != 6: return False
        elif seqid.count(' ') != 0: return False 
        elif seqid.count('/') != 1: return False
        return True
